...
theme.notification_fg_focus = theme.pink
theme.notification_bg_focus = theme.bg3
...
local clock = awful.widget.watch("date +' %d %b %a %I:%M '", 10, function(widget, stdout)
	widget:set_markup("  " .. markup.font(theme.font, markup(theme.pink, stdout)))
end)
...
local temp = lain.widget.temp({
	settings = function()
		widget:set_markup(markup.fontfg(theme.font, "#bb9af7", coretemp_now .. "°C "))
	end,
})
...
local memory = lain.widget.mem({
	settings = function()
		widget:set_markup(markup.fontfg(theme.font, "#7dcfff", mem_now.used .. "M "))
	end,
})
...
function theme.at_screen_connect(s)
	...
	s.mywibox:setup({
		...
		{ -- Right widgets
			...
			-- wibox.container.background(mysep(theme.bg2, right_tri), theme.bg4),
			wibox.container.background(net_speed_widget(), theme.bg3),
			wibox.container.background(mysep(theme.bg2, right_tri), theme.bg4),
			wibox.container.background(
				wibox.container.margin(wibox.widget({ memicon, memory.widget, layout = wibox.layout.align.horizontal })),
				theme.bg3
			),
			wibox.container.background(mysep(theme.bg2, right_tri), theme.bg4),
			wibox.container.background(
				wibox.container.margin(wibox.widget({ tempicon, temp.widget, layout = wibox.layout.align.horizontal })),
				theme.bg3
			),
			wibox.container.background(mysep(theme.bg3, left_tri), theme.bg4),

			wibox.container.background(
				wibox.container.margin(wibox.widget({ cpuicon, cpu.widget, layout = wibox.layout.align.horizontal })),
				theme.bg3
			),
